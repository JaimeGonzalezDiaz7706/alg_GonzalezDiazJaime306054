...
public class AlmacenajeContenedoresRyP {
    ...
    /**
     * El heurístico que usamos es aplicar una suma de los
     * pesos restantes en cada vuelta, luego sumamos
     * a la suma de los restantes la cota minima y si
     * ese valor es mayor que el mejor número entonces es 
     * el nuevo mejor
     */
    public static void resolver(int indice, int numContenedoresUsados) {
        int sumaRes = 0;
        for (int i = indice; i < tamanos.length; i++) {
            sumaRes += tamanos[i];
        }

        int cotaMinima = (sumaRes + capacidadMax - 1) / capacidadMax;
        if (numContenedoresUsados + cotaMinima >= mejorNum) {
            return;
        }
